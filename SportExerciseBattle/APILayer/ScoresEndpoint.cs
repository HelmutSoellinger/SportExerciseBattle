...
namespace SportExerciseBattle.APILayer
{
    ...
    {
        private ScoresDAO scoresDAO = new ScoresDAO(); // Create an instance of StatsDAO

        public bool HandleRequest(HttpRequest rq, HttpResponse rs)
        {
            if (rq.Method == HttpMethod.GET)
            {
                GetScores(rq, rs, scoresDAO); // Delegate the task to GetStats method
                return true;
            }
            return false;
        }
        ...
        public void GetScores(HttpRequest rq, HttpResponse rs, ScoresDAO scoresDAO)
        {
            try
            {
                ...
                var statsList = scoresDAO.GetScores();
                ...
                rs.Content = JsonSerializer.Serialize(statsList);
